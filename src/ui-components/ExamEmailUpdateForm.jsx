...
export default function ExamEmailUpdateForm(props) {
  const {
    id: idProp,
    examEmail: examEmailModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = examEmailRecord
      ? { ...initialValues, ...examEmailRecord }
      : initialValues;
    setExamLink(cleanValues.examLink);
  ...
  const [examEmailRecord, setExamEmailRecord] =
    React.useState(examEmailModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ExamEmail, idProp)
        : examEmailModelProp;
      setExamEmailRecord(record);
    };
    queryData();
  }, [idProp, examEmailModelProp]);
  React.useEffect(resetStateValues, [examEmailRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ExamEmail.copyOf(examEmailRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ExamEmailUpdateForm")}
      {...rest}
    >
      <TextField
        label="Exam link"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || examEmailModelProp)}
